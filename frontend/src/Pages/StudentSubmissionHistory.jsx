        ...
        if (!cancelled) {
          setError(
            requestError.response?.data?.message ||
              'Unable to load your classes for submission history.',
          );
        }
      } finally {
        if (!cancelled) {
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className={`mt-3 text-sm md:text-base ${mutedText}`}>
              Browse all your submissions with the exact status and date history shown in
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {[
              ...
              <div
                key={label}
                className={`rounded-2xl border p-4 ${isDark ? 'bg-black/20 border-white/10' : 'bg-gray-50 border-gray-200'}`}
              >
        ...
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            ...
            <p className={`text-sm ${mutedText}`}>
              Narrow your history by class, assignment, date, or current review status.
            </p>
          </div>
        </div>
