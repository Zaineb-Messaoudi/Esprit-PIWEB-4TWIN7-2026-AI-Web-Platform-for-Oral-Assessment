...
import { getAiAnalysis } from '../api/aiAnalysis';
...
const AI_STATUS = {
  ...
  COMPLETED:  'completed',
  FAILED:     'failed',   // terminal — Python pipeline error
  ERROR:      'error',    // terminal — network / NestJS error
};
      ...
      {Array.from({ length: 32 }).map((_, i) => (
        <div
          ...
          className={`w-[3px] rounded-full ${
            playing ? 'bg-red-400' : 'bg-white/20'
          }`}
          style={{
...
function MediaPlayer({ src, isAudio, isDark }) {
  ...
  if (!src)
    return (
      <div
        className={`flex items-center justify-center h-40 rounded-2xl border text-sm ${
          isDark
  ...
  return (
    <div
      className={`rounded-2xl border overflow-hidden ${
        isDark
          ? 'border-white/10 bg-gradient-to-br from-black/40 to-white/5'
      ...
      {isAudio && (
        <div
          className={`flex items-center justify-center py-12 px-6 ${
            isDark
              ? 'bg-gradient-to-br from-red-900/20 to-black/40'
...
/* ─── AI Analysis Status UI ────────────────────────────────── */
function AIAnalysisStatus({ status, isDark }) {
  const states = {
    [AI_STATUS.LOADING]: {
      icon: <RefreshCw size={16} className="animate-spin text-red-400" />,
    ...
    [AI_STATUS.PENDING]: {
      ...
      description:
        'Your submission is in the queue. Analysis will begin shortly.',
      cls: isDark
    ...
    [AI_STATUS.FAILED]: {
      ...
      description:
        'The AI could not process this submission. Please try resubmitting or contact support.',
      cls: isDark
    ...
    [AI_STATUS.ERROR]: {
      ...
      description:
        'Something went wrong while fetching the AI analysis. Please try again later.',
      cls: isDark
        ? 'border-red-500/20 bg-red-500/10 text-red-300'
        : 'border-red-200 bg-red-50 text-red-700',
    },
  ...
  return (
    <div className={`rounded-2xl border px-4 py-4 ${state.cls}`}>
...
export default function StudentSubmissionDetail() {
  ...
  const [aiStatus,   setAiStatus]   = useState(AI_STATUS.LOADING);
  ...
  /* ── AI analysis fetch + polling ── */
  const fetchAnalysis = useCallback(async () => {
    try {
      const res = await getAiAnalysis(id);
      // res shape: { status: 'pending' | 'processing' | 'completed' | 'failed', data: {...} | null }
      setAiStatus(res.status);

      if (res.status === AI_STATUS.COMPLETED) {
        setAiAnalysis(res.data);
        stopPolling();
      }

      // Stop polling on any terminal status so we don't hammer the server
  ...
  useEffect(() => {
    ...
    fetchAnalysis();
    pollingRef.current = setInterval(fetchAnalysis, POLL_INTERVAL_MS);

    return () => stopPolling();
  }, [id, fetchAnalysis, stopPolling]);
  ...
  const isPolling =
    aiStatus === AI_STATUS.PENDING || aiStatus === AI_STATUS.PROCESSING;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Status banner ────────────────────────────────────── */}
      <section
        className={`rounded-3xl border p-5 flex items-center justify-between flex-wrap gap-4 ${cardBase}`}
      >
        <div className="flex items-center gap-4">
          <div
            className={`rounded-2xl p-3 ${
              isDark ? 'bg-white/10' : 'bg-red-50'
            }`}
          >
            {isAudio ? (
              <FileAudio
          ...
          <div>
            <p className="font-semibold">
              {submission?.title || submission?.assignmentTitle}
            </p>
        ...
        <div className="flex items-center gap-3">
          ...
          <span
            className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold border ${statusMeta.cls}`}
          >
            <StatusIcon size={12} />
            {statusMeta.label}
          </span>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_300px]">
          ...
          <section className={`rounded-3xl border p-6 ${cardBase}`}>
            ...
            <div className="space-y-3">
              {[
                ...
                [
                  'Submitted at',
                  isSubmitted ? fmtDate(submission?.submittedAt) : '—',
                ],
              ].map(([label, value]) => (
                <div
                  key={label}
                  className={`flex justify-between items-start gap-2 py-2 border-b last:border-0 ${
          ...
          {submission?.class && (
            <section className={`rounded-3xl border p-6 ${cardBase}`}>
              ...
              <p className={`text-xs mt-1 ${subtleText}`}>
                {[
                  submission.class.academicYear,
                  submission.class.semester,
                ]
                  .filter(Boolean)
                  .join(' • ') || '—'}
              </p>
        ...
        {/* ── AI Analysis ─────────────────────────────────────── */}
        <section
          className={`rounded-3xl border p-6 xl:col-span-2 ${cardBase}`}
        >
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              <h3 className="text-lg font-semibold">AI Speech Analysis</h3>
              <p className={`text-xs ${subtleText}`}>
            ...
            {isPolling && (
              <span
                className={`ml-auto inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs border ${
                  isDark
                    ? 'border-blue-500/20 bg-blue-500/10 text-blue-300'
          ...
          {aiStatus === AI_STATUS.COMPLETED && aiAnalysis ? (
            <AIAnalysisReport analysis={aiAnalysis} />
          ) : (
            <AIAnalysisStatus status={aiStatus} isDark={isDark} />
          )}
        </section>
