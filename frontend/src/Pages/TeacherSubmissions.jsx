...
function TeacherSubmissions() {
  ...
  const selectedClass = classes.find((classItem) => classItem.id === filters.classId) || null;
  const studentOptions = selectedClass?.students || [];

  useEffect(() => {
    if (!filters.classId) {
      setFilters((current) => (current.studentId ? { ...current, studentId: '' } : current));
      return;
    }

    const studentStillExists = studentOptions.some(
      (student) => student.id === filters.studentId,
    );

    if (!studentStillExists && filters.studentId) {
        ...
        if (!cancelled) {
          setError(
            requestError.response?.data?.message ||
              'Unable to load classes for the submission dashboard.',
          );
        }
      } finally {
        if (!cancelled) {
  ...
  useEffect(() => {
    loadOverview();
    // We intentionally fetch on mount as soon as classes are available.
  ...
  const loadOverview = async (sourceFilters = filters) => {
    ...
    try {
      ...
      setError(
        requestError.response?.data?.message ||
          'Unable to load submission data for the selected filters.',
      );
    } finally {
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className={`mt-3 text-sm md:text-base ${mutedText}`}>
              Review every submission in your classes, narrow the list by student or
              assignment, and spot who still has not submitted.
            </p>
          </div>

          <div className={`grid gap-3 sm:grid-cols-2 xl:grid-cols-4 ${isDark ? '' : ''}`}>
            {[
              {
                label: 'Total submissions',
                value: summary.total,
                icon: FileSearch,
                accent: isDark ? 'text-red-200' : 'text-red-700',
              },
              {
                label: 'Pending',
                value: summary.pending,
                icon: Clock3,
                accent: isDark ? 'text-amber-200' : 'text-amber-700',
              },
              {
                label: 'In progress',
                value: summary.inProgress,
                icon: LoaderCircle,
                accent: isDark ? 'text-sky-200' : 'text-sky-700',
              },
              {
                label: 'Evaluated',
                value: summary.evaluated,
                icon: CheckCircle2,
                accent: isDark ? 'text-emerald-200' : 'text-emerald-700',
              },
            ].map(({ label, value, icon: Icon, accent }) => (
              <div
                key={label}
                className={`min-w-[150px] rounded-2xl border p-4 ${isDark ? 'bg-black/20 border-white/10' : 'bg-gray-50 border-gray-200'}`}
              >
      ...
      <section className={`rounded-3xl border p-6 ${cardBase}`}>
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-semibold">Filter submissions</h3>
            <p className={`text-sm ${mutedText}`}>
              Match the image requirements by class, student, assignment, date, or status.
            </p>
          </div>
        </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <label className="space-y-2 text-sm">
              ...
              <input
                ...
                onChange={handleFilterChange}
                placeholder="e.g. Oral presentation 1"
                className={`w-full rounded-2xl border px-4 py-3 outline-none transition ${inputBase}`}
              />
            </label>
      ...
      {filters.classId && filters.assignmentTitle.trim() ? (
        <section className={`rounded-3xl border p-6 ${cardBase}`}>
            ...
            <div>
              ...
              <p className={`text-sm ${mutedText}`}>
                ...
                <span className="font-semibold">
                  {missingData.class?.name || selectedClass?.name || 'selected class'}
                </span>{' '}
                who have not submitted{' '}
                <span className="font-semibold">{filters.assignmentTitle.trim()}</span>.
              </p>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {[
              ['Class roster', missingData.counts.totalStudents],
              ['Submitted', missingData.counts.submitted],
              ['Still missing', missingData.counts.missing],
            ].map(([label, value]) => (
              <div
                key={label}
                className={`rounded-2xl border p-4 ${isDark ? 'border-white/10 bg-black/20' : 'border-gray-200 bg-gray-50'}`}
              >
                <p className={`text-sm ${subtleText}`}>{label}</p>
          ...
          <div className="mt-5 overflow-hidden rounded-2xl border">
            <div className={`grid grid-cols-[1.4fr_1fr_1fr] px-4 py-3 text-sm font-semibold ${
            ...
            {missingData.missingStudents.length ? (
              missingData.missingStudents.map((student) => (
                <div
                  key={student.id}
                  className={`grid grid-cols-[1.4fr_1fr_1fr] gap-4 px-4 py-3 text-sm ${
                    isDark ? 'border-t border-white/10 text-gray-200' : 'border-t border-gray-200 text-gray-700'
                  }`}
                >
              ...
              <div className={`px-4 py-6 text-sm ${mutedText}`}>
                Everyone in this class has already submitted for the selected assignment.
              </div>
            )}
          </div>
        </section>
      ...
      <section className={`rounded-3xl border ${cardBase}`}>
        <div className="flex items-center justify-between border-b px-6 py-4">
          <div>
            ...
            <p className={`text-sm ${mutedText}`}>
              All matching records with class, student, status, and timestamp.
            </p>
          </div>
          ...
          {loadingResults ? (
            ...
            <div className={`inline-flex items-center gap-2 text-sm ${mutedText}`}>
              ...
              {overview.length} result{overview.length === 1 ? '' : 's'}
            </div>
          )}
        </div>
