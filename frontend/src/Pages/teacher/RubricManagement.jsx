...
const RubricManagement = () => {
  ...
  const [formData, setFormData] = useState({
    name: '',
    description: '',
    criteria: [{ key: crypto.randomUUID(), name: '', description: '', maxScore: 5 }]
  });
  ...
  const loadRubrics = async () => {
    ...
    try {
      ...
      console.error("Erreur rubriques");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddCriterion = () => {
    setFormData({
      ...formData,
      criteria: [...formData.criteria, { key: crypto.randomUUID(), name: '', description: '', maxScore: 5 }]
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (currentId) {
        await assignmentService.updateRubric(currentId, {
  ...formData,
  ...
  const resetForm = () => {
    setFormData({ name: '', description: '', criteria: [{ key: crypto.randomUUID(), name: '', description: '', maxScore: 5 }] });
    setIsEditing(false);
    setCurrentId(null);
  };
                ...
                {formData.criteria.map((c, idx) => (
                  <div key={idx} className={`p-5 rounded-2xl border group relative ${isDark ? 'bg-white/5 border-white/5' : 'bg-gray-50 border-gray-100'}`}>
                    <div className="flex flex-col md:flex-row gap-4 mb-3">
                      ...
                      <button type="button" onClick={() => handleRemoveCriterion(c.key)} className="text-red-400 hover:text-red-500 self-center">
                        <Trash2 size={20} />
                      </button>
                    </div>
                    <textarea 
