...
const fetchWithAuth = async (url, options = {}) => {
  const token = localStorage.getItem('token'); // Change this if your token key is different
...
const Analytics = ({ classId }) => {
  ...
  useEffect(() => {
    if (classId) return;  // activeClassId is already initialized from the prop
    if (!instructorId) return;
    
    fetchWithAuth(`${BASE_URL}/classes/instructor/${instructorId}`)
      .then(r => {
      ...
      .catch(err => {
        ...
        setError('Failed to load classes. Please check your authentication.');
      });
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchWithAuth(`${BASE_URL}/ai-analyses/class/${activeClassId}/stats`).then(r => {
        if (!r.ok) throw new Error(`HTTP error! status: ${r.status}`);
        return r.json();
      }),
      fetchWithAuth(`${BASE_URL}/classes/${activeClassId}/students`).then(r => {
        if (!r.ok) throw new Error(`HTTP error! status: ${r.status}`);
        return r.json();
      }),
    ])
  ...
  useEffect(() => {
    if (!selectedStudent) {
      startTransition(() => setStudentStats(null));
      return;
    }
    
    startTransition(() => setLoadingStudent(true));
    
    fetchWithAuth(`${BASE_URL}/ai-analyses/student/${selectedStudent}/stats`)
      .then(r => {
        if (!r.ok) throw new Error(`HTTP error! status: ${r.status}`);
        return r.json();
      })
