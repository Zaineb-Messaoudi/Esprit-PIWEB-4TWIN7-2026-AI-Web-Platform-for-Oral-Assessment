...
const AIAnalysisSection = memo(({ submissionId, isDark, isOpen, onToggle }) => {
  ...
  return (
    <div className={`rounded-xl border ${isDark ? 'border-white/10' : 'border-gray-200'}`}>
      ...
      {isOpen && (
        <div className={`px-4 pb-4 space-y-4 border-t ${isDark ? 'border-white/10' : 'border-gray-100'}`}>
          {!loaded ? (
            <div className="pt-4 space-y-2">
              {[1,2,3].map(i => <div key={i} className={`h-12 rounded-xl animate-pulse ${isDark ? 'bg-white/5' : 'bg-gray-100'}`} />)}
            </div>
          ) : !analysis || analysis.statusCode === 404 ? (
            ...
            <div className="pt-3 space-y-3">
              ...
              <StatRow icon={Volume2} label="Avg Pause Duration" value={analysis.pauseDuration?.average} unit="sec"   isDark={isDark} />
              ...
              {analysis.suggestions?.length > 0 && (
                <div>
                  <p className={`text-xs font-semibold uppercase tracking-wider mb-2 flex items-center gap-1 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
